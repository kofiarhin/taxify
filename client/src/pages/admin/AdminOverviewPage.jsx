      ...
      {isLoading && (
        <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
          {Array.from({ length: 8 }).map((_, index) => (
            <div key={index} className="h-28 animate-pulse rounded-[1.8rem] bg-white/6" />
          ))}
        </div>
      )}
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
            ...
            <KpiCard
              ...
              accent={data.queueCount > 0 ? "text-amber-300" : "text-white"}
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
            ...
            <KpiCard
              ...
              value={data.suspendedDrivers}
              accent={data.suspendedDrivers > 0 ? "text-red-300" : "text-white"}
            />
            <KpiCard
              label="Open complaints"
              value={data.openComplaints}
              accent={data.openComplaints > 0 ? "text-amber-300" : "text-white"}
            />
          </div>
